...
{ 
    ...
    {
        $form_data = $this->validate(
            $this->login_model, 'validate_change_email', 'form_errors_auth'
        );
        if (null===$form_data) return;
     ...
     */
    protected function validate($model, $callback, $errors) {

        form::$data = $this->view->form_data = array_merge(
            $model->loaded ?  $model->as_array() : array(),
            ('post' == request::method()) ? 
        ...
        $is_valid = call_user_func_array(
            array($model, $callback), array(&form::$data)
        );

        if (!$is_valid) {
            form::$errors = $this->view->form_errors = form::$data->errors($errors);
            return null;
        }
